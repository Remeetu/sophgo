...
void LLama2::load_sentencepiece(std::string tokenizer) {
  printf("Load %s ... ", tokenizer.c_str());
  auto status = sentencepiece.Load(tokenizer);
  if (!status.ok()) {
...
void LLama2::init(const std::vector<int> &devices) {
  std::regex fileSuffixRegexModel(
      R"(\.model)"); // 正则表达式匹配以 ".model" 结尾的文件
  std::regex fileSuffixRegexBmodel(
      R"(\.bmodel)"); // 正则表达式匹配以 ".bmodel" 结尾的文件

  const std::string targetDir =
      "/workspace/PaddleCustomDevice/backends/sophgo";
  std::string tokenizer, model;
  for (const auto &entry : std::filesystem::directory_iterator(targetDir)) {
    if (std::filesystem::is_regular_file(entry) &&
        std::regex_search(entry.path().filename().string(),
                          fileSuffixRegexModel)) {
      tokenizer = entry.path();
      std::cout << "model Path: " << entry.path() << std::endl;
    }
    if (std::filesystem::is_regular_file(entry) &&
        std::regex_search(entry.path().filename().string(),
                          fileSuffixRegexBmodel)) {
      model = entry.path();
      std::cout << "model Path: " << entry.path() << std::endl;
    }
  }
